"""
Tests in here check that errors that should be raised by the NagaitsevIBS class
are indeed raised.
"""
import pytest
...
def test_growth_rates_raises_if_no_integrals(xtrack_ps_injection_protons, caplog):
    """
    Checking that NagaitsevIBS.growth_rates raises and error if the calculation
    of the integrals has not been performed beforehand.
    """
    # --------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------
    # Check the error is raised by .growth_rates
    ...
    for record in caplog.records:  # check the logging message
        ...
        assert (
            "Attempted to compute growth rates without having computed Nagaitsev integrals" in record.message
        )


def test_emittance_evolution_raises_if_no_growth_rates(xtrack_ps_injection_protons, caplog):
    """
