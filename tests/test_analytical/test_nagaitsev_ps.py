...
def test_ps_injection_protons_vs_old(madx_ps_injection_protons, xtrack_ps_injection_protons):
    # --------------------------------------------------------------------
...
def test_ps_injection_protons_vs_madx(madx_ps_injection_protons):
    # --------------------------------------------------------------------
    ...
    assert np.isclose(IBS.ibs_growth_rates.Tx, mad_Tx, atol=0, rtol=4e-2)
    assert np.isclose(IBS.ibs_growth_rates.Ty, mad_Ty, atol=0, rtol=7.5e-2)
    assert np.isclose(IBS.ibs_growth_rates.Tz, mad_Tz, atol=0, rtol=1.5e-2)


def test_ps_injection_ions_vs_old(madx_ps_injection_ions, xtrack_ps_injection_ions):
    # --------------------------------------------------------------------
    # Get MAD-X setup - only want for the params
    madx, params = madx_ps_injection_ions  # fully set up from the config file
    # --------------------------------------------------------------------
    # Load equivalent xsuite line
    line: xt.Line = xtrack_ps_injection_ions
    twiss = line.twiss(method="4d")
    opticsparams = OpticsParameters(twiss)
    beamparams = BeamParameters.from_madx(madx)
    beamparams.n_part = madx.sequence.ps.beam.npart  # as line.particle_ref is only 1 particle
...
def test_ps_injection_ions_vs_madx(madx_ps_injection_ions):
    # --------------------------------------------------------------------
    # Get the growth rates from MAD-X
    madx, params = madx_ps_injection_ions  # fully set up from the config file
    mad_Tx, mad_Ty, mad_Tz = get_madx_ibs_growth_rates(madx)
    ...
    # Compare the results to MAD-X - deviations are ok as long as we are
    # close to the old code's results (test above)
    assert np.isclose(IBS.ibs_growth_rates.Tx, mad_Tx, atol=0, rtol=35e-2)
