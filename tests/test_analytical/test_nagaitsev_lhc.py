...
def test_lhc_injection_protons_vs_old(madx_lhc_injection_protons, xtrack_lhc_injection_protons):
    # --------------------------------------------------------------------
    ...
    line: xt.Line = xtrack_lhc_injection_protons
...
def test_lhc_injection_protons_vs_madx(madx_lhc_injection_protons):
    # --------------------------------------------------------------------
    ...
    assert np.isclose(IBS.ibs_growth_rates.Ty, mad_Ty, atol=0, rtol=1.5e-2)  # bit more margin in vertical
...
def test_lhc_top_protons_vs_old(madx_lhc_top_protons, xtrack_lhc_top_protons):
    # --------------------------------------------------------------------
    # Get MAD-X setup - only want for the params
    madx, params = madx_lhc_top_protons  # fully set up from the config file
    # --------------------------------------------------------------------
    # Load equivalent xsuite line
    line: xt.Line = xtrack_lhc_top_protons
    twiss = line.twiss(method="4d")
    opticsparams = OpticsParameters(twiss)
    beamparams = BeamParameters.from_madx(madx)
    beamparams.n_part = madx.sequence.lhcb1.beam.npart  # as line.particle_ref is only 1 particle
...
def test_lhc_top_protons_vs_madx(madx_lhc_top_protons):
    # --------------------------------------------------------------------
    # Get the growth rates from MAD-X
    madx, params = madx_lhc_top_protons  # fully set up from the config file
    mad_Tx, mad_Ty, mad_Tz = get_madx_ibs_growth_rates(madx)
...
def test_lhc_top_ions_vs_old(madx_lhc_top_ions, xtrack_lhc_top_ions):
    # --------------------------------------------------------------------
    # Get MAD-X setup - only want for the params
    madx, params = madx_lhc_top_ions  # fully set up from the config file
    # --------------------------------------------------------------------
    # Load equivalent xsuite line
    line: xt.Line = xtrack_lhc_top_ions
    twiss = line.twiss(method="4d")
    opticsparams = OpticsParameters(twiss)
    beamparams = BeamParameters.from_madx(madx)
    beamparams.n_part = madx.sequence.lhcb1.beam.npart  # as line.particle_ref is only 1 particle
...
def test_lhc_top_ions_vs_madx(madx_lhc_top_ions):
    # --------------------------------------------------------------------
    # Get the growth rates from MAD-X
    madx, params = madx_lhc_top_ions  # fully set up from the config file
    mad_Tx, mad_Ty, mad_Tz = get_madx_ibs_growth_rates(madx)
