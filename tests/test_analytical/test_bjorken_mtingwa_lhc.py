...
def test_lhc_injection_protons(madx_lhc_injection_protons, bunched):
    # --------------------------------------------------------------------
    ...
    assert np.isclose(IBS.ibs_growth_rates.Ty, mad_Ty, atol=0, rtol=2.5e-2)
...
@pytest.mark.parametrize("bunched", [True, False])
def test_lhc_top_protons(madx_lhc_top_protons, bunched):
    # --------------------------------------------------------------------
    # Get the growth rates from MAD-X
    ...
    # --------------------------------------------------------------------
    # Now we compare values to MAD-X - need slightly higher margins
    assert np.isclose(IBS.ibs_growth_rates.Tx, mad_Tx, atol=0, rtol=2.5e-2)
    assert np.isclose(IBS.ibs_growth_rates.Ty, mad_Ty, atol=0, rtol=3.8e-2)
    assert np.isclose(IBS.ibs_growth_rates.Tz, mad_Tz, atol=0, rtol=1e-2)


@pytest.mark.parametrize("bunched", [True, False])
def test_lhc_top_ions(madx_lhc_top_ions, bunched):
    # --------------------------------------------------------------------
    # Get the growth rates from MAD-X
    ...
    # Now we compare values to MAD-X
    assert np.isclose(IBS.ibs_growth_rates.Tx, mad_Tx, atol=0, rtol=1e-2)
    assert np.isclose(IBS.ibs_growth_rates.Ty, mad_Ty, atol=0, rtol=4.5e-2)  # bit more margin in vertical
    assert np.isclose(IBS.ibs_growth_rates.Tz, mad_Tz, atol=0, rtol=1e-2)
