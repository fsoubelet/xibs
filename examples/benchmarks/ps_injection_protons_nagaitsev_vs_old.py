...
in the PS, for protons at injection energy.
"""
import time
import warnings
...
filepath = xibs_repo / "tests" / "inputs" / "lines" / "ps_injection_protons.json"
line = xt.Line.from_json(filepath)
p0 = line.particle_ref
line.build_tracker()
twiss = line.twiss(method="4d")

# ----- File and parameters (taken from lhc_injection_protons.yml) ----- #

filepath = Path(__file__).parent.parent.parent 
geom_epsx = 2.685e-07
geom_epsy = 1.678e-07
sig_delta = 0.0002740293584929326
bunch_length_m = 0.5684116447339836
...
assert np.isclose(MIBS.Ixx, BMIBS.ibs_growth_rates.Tx, rtol=5e-2)
assert np.isclose(MIBS.Iyy, BMIBS.ibs_growth_rates.Ty, rtol=5e-2)
assert np.isclose(MIBS.Ipp, BMIBS.ibs_growth_rates.Tz, rtol=5e-2)
